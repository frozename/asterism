...
import { adapters, buildRegistry } from '../src/adapters/index.js';
...
// ---- the same sweeps, with the second (deliberately poor) adapter enabled ----

test('with the fake adapter enabled: the registry is a Map keyed by each entry\'s own id', () => {
  const registry = buildRegistry({ ASTERISM_FAKE_ROOT: '/x' });
  assert.ok(registry instanceof Map);
  assert.ok(registry.size > 0, 'expected at least one registered adapter');
...
test('with the fake adapter enabled: every adapter lists non-empty, frozen, uniquely-named uppercase env markers', () => {
  const registry = buildRegistry({ ASTERISM_FAKE_ROOT: '/x' });
...
test('with the fake adapter enabled: every adapter exposes a non-empty static probe symbol list', () => {
  const registry = buildRegistry({ ASTERISM_FAKE_ROOT: '/x' });
  for (const adapter of registry.values()) {
    assert.ok(adapter.staticProbe.symbols.length > 0, `${adapter.id} should list at least one symbol`);
  }
...
test('with the fake adapter enabled: binaryCandidates is pure and scopes every directory under the given home', () => {
  const registry = buildRegistry({ ASTERISM_FAKE_ROOT: '/x' });
  for (const adapter of registry.values()) {
    const candidates = adapter.staticProbe.binaryCandidates('/h');
    assert.ok(candidates.length > 0, `${adapter.id} should return at least one candidate`);
